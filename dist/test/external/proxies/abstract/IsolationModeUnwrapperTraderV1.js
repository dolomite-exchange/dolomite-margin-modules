...
describe('IsolationModeUnwrapperTraderV1', () => {
    ...
    before(async () => {
        ...
        unwrapper = await (0, dolomite_utils_1.createContractWithAbi)(types_1.TestIsolationModeUnwrapperTraderV1__factory.abi, types_1.TestIsolationModeUnwrapperTraderV1__factory.bytecode, [otherToken.address, factory.address, core.dolomiteMargin.address]);
    ...
    describe('Call and Exchange for non-liquidation sale', () => {
        it('should work when called with the normal conditions', async () => {
            ...
            const actions = await unwrapper.createActionsForUnwrappingForLiquidation(solidAccountId, liquidAccountId, Addresses_1.ZERO_ADDRESS, Addresses_1.ZERO_ADDRESS, otherMarketId, underlyingMarketId, no_deps_constants_1.ZERO_BI, amountWei);
    ...
    describe('#callFunction', () => {
        ...
        it('should fail if account.owner param is not a vault', async () => {
            const dolomiteMarginCaller = await (0, utils_2.impersonate)(core.dolomiteMargin.address, true);
            await core.dolomiteMargin.ownerSetGlobalOperator(core.hhUser5.address, true);
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginCaller).callFunction(core.hhUser5.address, { owner: core.hhUser1.address, number: defaultAccountNumber }, utils_1.defaultAbiCoder.encode(['uint256'], [amountWei])), `IsolationModeUnwrapperTraderV1: Account owner is not a vault <${core.hhUser1.address.toLowerCase()}>`);
        });
        it('should fail if transferAmount param is 0', async () => {
            const dolomiteMarginCaller = await (0, utils_2.impersonate)(core.dolomiteMargin.address, true);
            await core.dolomiteMargin.ownerSetGlobalOperator(core.hhUser5.address, true);
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginCaller).callFunction(core.hhUser5.address, { owner: vault.address, number: defaultAccountNumber }, utils_1.defaultAbiCoder.encode(['uint256'], [no_deps_constants_1.ZERO_BI])), 'IsolationModeUnwrapperTraderV1: Invalid transfer amount');
        });
        it('should fail if vault underlying balance is less than the transfer amount (ISF)', async () => {
            const dolomiteMarginCaller = await (0, utils_2.impersonate)(core.dolomiteMargin.address, true);
            await core.dolomiteMargin.ownerSetGlobalOperator(core.hhUser5.address, true);
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginCaller).callFunction(core.hhUser5.address, { owner: vault.address, number: defaultAccountNumber }, utils_1.defaultAbiCoder.encode(['uint256'], [amountWei.mul(111)])), `IsolationModeUnwrapperTraderV1: Insufficient balance <${amountWei.toString()}, ${amountWei.mul(111)
                .toString()}>`);
        });
    });
    describe('#exchange', () => {
        ...
        it('should fail if there is an insufficient input token balance', async () => {
            const dolomiteMarginImpersonator = await (0, utils_2.impersonate)(core.dolomiteMargin.address, true);
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, otherToken.address, factory.address, amountWei, abiCoder.encode(['uint256'], [amountWei])), `IsolationModeUnwrapperTraderV1: Insufficient input token <0, ${amountWei.toString()}>`);
        });
        it('should fail if there is an insufficient amount outputted', async () => {
            ...
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, otherToken.address, factory.address, amountWei, abiCoder.encode(['uint256'], [amountWei.mul(2)])), `IsolationModeUnwrapperTraderV1: Insufficient output amount <${amountWei.toString()}, ${amountWei.mul(2)
                .toString()}>`);
        });
    });
    describe('#token', () => {
    ...
    describe('#outputMarketId', () => {
        it('should work', async () => {
            (0, chai_1.expect)(await unwrapper.outputMarketId()).to.eq(otherMarketId);
        });
    });
    describe('#createActionsForUnwrappingForLiquidation', () => {
        it('should work for normal condition', async () => {
            ...
            const actions = await unwrapper.createActionsForUnwrappingForLiquidation(solidAccountId, liquidAccountId, solidUser.address, core.hhUser1.address, otherMarketId, underlyingMarketId, otherAmountWei, amountWei);
            ...
            const amountOut = await unwrapper.getExchangeCost(factory.address, core.tokens.usdc.address, amountWei, no_deps_constants_1.BYTES_EMPTY);
            (0, chai_1.expect)(actions[1].data).to.eq(abiCoder.encode(['uint', 'bytes'], [amountOut, no_deps_constants_1.BYTES_EMPTY]));
        });
    });
