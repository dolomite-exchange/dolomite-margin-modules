...
describe('IsolationModeWrapperTraderV1', () => {
    ...
    before(async () => {
        ...
        wrapper = await (0, dolomite_utils_1.createContractWithAbi)(types_1.TestIsolationModeWrapperTraderV1__factory.abi, types_1.TestIsolationModeWrapperTraderV1__factory.bytecode, [factory.address, core.dolomiteMargin.address]);
    ...
    describe('Call and Exchange for non-liquidation sale', () => {
        it('should work when called with the normal conditions', async () => {
            ...
            const actions = await wrapper.createActionsForWrapping(solidAccountId, liquidAccountId, Addresses_1.ZERO_ADDRESS, Addresses_1.ZERO_ADDRESS, underlyingMarketId, otherMarketId, no_deps_constants_1.ZERO_BI, otherAmountWei);
    ...
    describe('#exchange', () => {
        ...
        it('should fail if the output amount is insufficient', async () => {
            ...
            utils_1.defaultAbiCoder.encode(['uint256'], [amountWei.mul(2)])), `IsolationModeWrapperTraderV1: Insufficient output amount <${amountWei.toString()}, ${amountWei.mul(2)
                .toString()}>`);
        });
    });
    ...
    describe('#createActionsForWrapping', () => {
        it('should work for normal condition', async () => {
            ...
            const actions = await wrapper.createActionsForWrapping(solidAccountId, liquidAccountId, solidUser.address, core.hhUser1.address, underlyingMarketId, otherMarketId, otherAmountWei, amountWei);
            ...
            const amountOut = await wrapper.getExchangeCost(factory.address, otherToken.address, amountWei, no_deps_constants_1.BYTES_EMPTY);
            (0, chai_1.expect)(actions[0].data).to.eq(abiCoder.encode(['uint', 'bytes'], [amountOut, no_deps_constants_1.BYTES_EMPTY]));
        });
        it('should fail when output market is invalid', async () => {
            const solidAccount = 0;
            await (0, assertions_1.expectThrow)(wrapper.createActionsForWrapping(solidAccount, solidAccount, Addresses_1.ZERO_ADDRESS, Addresses_1.ZERO_ADDRESS, core.marketIds.weth, otherMarketId, no_deps_constants_1.ZERO_BI, amountWei), `IsolationModeWrapperTraderV1: Invalid output market <${core.marketIds.weth.toString()}>`);
        });
    });
});
