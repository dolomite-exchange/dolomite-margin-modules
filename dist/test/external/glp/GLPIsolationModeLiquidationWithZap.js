...
describe('GLPIsolationModeLiquidationWithZap', () => {
    ...
    before(async () => {
        ...
        heldMarketId = ethers_1.BigNumber.from(core.marketIds.dfsGlp);
        glpApiToken = {
    ...
    describe('Perform liquidation with full integration', () => {
        it('should work when liquid account is borrowing the output token (USDC)', async () => {
            ...
            const heldUpdatedWithReward = await newAccountValues[1].value.mul(liquidationSpreadNumerator)
                ...
                .div(glpPrice.value);
            const zapOutputs = await zap.getSwapExactTokensForTokensParams(glpApiToken, (0, liquidation_utils_1.toZapBigNumber)(heldUpdatedWithReward), core.apiTokens.usdc, (0, liquidation_utils_1.toZapBigNumber)(usdcDebtAmount), core.hhUser5.address);
        ...
        it('should work when liquid account is borrowing a different output token (WETH)', async () => {
            ...
            const wethLiquidatorBalanceBefore = await core.tokens.weth.connect(core.hhUser1)
                .balanceOf(core.liquidatorProxyV4.address);
            const zapOutputs = await zap.getSwapExactTokensForTokensParams(glpApiToken, (0, liquidation_utils_1.toZapBigNumber)(heldUpdatedWithReward), core.apiTokens.weth, (0, liquidation_utils_1.toZapBigNumber)(wethDebtAmount), core.hhUser5.address);
