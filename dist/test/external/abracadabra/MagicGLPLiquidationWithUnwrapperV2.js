...
describe('MagicGLPLiquidationWithUnwrapperV2', () => {
    ...
    before(async () => {
        ...
        unwrapper = await (0, abracadabra_1.createMagicGLPUnwrapperTraderV2)(core);
    ...
    describe('Perform liquidation with full integration', () => {
        it('should work when liquid account is borrowing the output token (USDC)', async () => {
            ...
            const txResult = await (0, liquidation_utils_1.liquidateV4WithLiquidityToken)(core, solidAccountStruct, liquidAccountStruct, [heldMarketId, core.marketIds.usdc], [no_deps_constants_1.SELL_ALL, no_deps_constants_1.LIQUIDATE_ALL], unwrapper);
            ...
            await (0, assertions_1.expectWalletBalance)(core.liquidatorProxyV4.address, magicGlp, no_deps_constants_1.ZERO_BI);
        ...
        it('should work when liquid account is borrowing a different output token (WETH)', async () => {
            ...
            const heldUpdatedWithReward = await newAccountValues[1].value.mul(liquidationSpreadNumerator)
                ...
                .div(magicGlpPrice.value);
            const usdcOutputAmount = await unwrapper.getExchangeCost(magicGlp.address, core.tokens.usdc.address, heldUpdatedWithReward, no_deps_constants_1.BYTES_EMPTY);
            const { calldata: paraswapCalldata, outputAmount: wethOutputAmount } = await (0, liquidation_utils_1.getCalldataForParaswap)(usdcOutputAmount, core.tokens.usdc, 6, no_deps_constants_1.ONE_BI, core.tokens.weth, 18, core.hhUser5, core.paraswapTrader, core);
            ...
            const wethLiquidatorBalanceBefore = await core.tokens.weth.connect(core.hhUser1)
                .balanceOf(core.liquidatorProxyV4.address);
            const isSuccessful = await (0, liquidation_utils_1.checkForParaswapSuccess)((0, liquidation_utils_1.liquidateV4WithLiquidityToken)(core, solidAccountStruct, liquidAccountStruct, [heldMarketId, core.marketIds.usdc, core.marketIds.weth], [no_deps_constants_1.SELL_ALL, usdcOutputAmount, no_deps_constants_1.LIQUIDATE_ALL], unwrapper, no_deps_constants_1.BYTES_EMPTY, (0, liquidation_utils_1.getParaswapTraderParamStruct)(core, paraswapCalldata), no_deps_constants_1.NO_EXPIRY));
            if (!isSuccessful) {
                return;
            }
            await (0, assertions_1.expectProtocolBalance)(core, solidAccountStruct.owner, solidAccountStruct.number, heldMarketId, no_deps_constants_1.ZERO_BI);
    ...
    describe('Perform expiration with full integration', () => {
        ...
        it('should work when expired account is borrowing a different output token (WETH)', async () => {
            ...
            const usdcOutputAmount = await unwrapper.getExchangeCost(magicGlp.address, core.tokens.usdc.address, heldUpdatedWithReward, no_deps_constants_1.BYTES_EMPTY);
            const { calldata: paraswapCalldata, outputAmount: wethOutputAmount } = await (0, liquidation_utils_1.getCalldataForParaswap)(usdcOutputAmount, core.tokens.usdc, 6, wethDebtAmount, core.tokens.weth, 18, core.hhUser5, core.paraswapTrader, core);
            const usdcLiquidatorBalanceBefore = await core.tokens.usdc.connect(core.hhUser1)
                .balanceOf(core.liquidatorProxyV4.address);
            const wethLiquidatorBalanceBefore = await core.tokens.weth.connect(core.hhUser1)
                .balanceOf(core.liquidatorProxyV4.address);
            const isSuccessful = (0, liquidation_utils_1.checkForParaswapSuccess)((0, liquidation_utils_1.liquidateV4WithLiquidityToken)(core, solidAccountStruct, liquidAccountStruct, [heldMarketId, core.marketIds.usdc, core.marketIds.weth], [no_deps_constants_1.SELL_ALL, usdcOutputAmount, no_deps_constants_1.LIQUIDATE_ALL], unwrapper, no_deps_constants_1.BYTES_EMPTY, (0, liquidation_utils_1.getParaswapTraderParamStruct)(core, paraswapCalldata), expiry));
            if (!isSuccessful) {
                return;
            }
            await (0, assertions_1.expectProtocolBalance)(core, solidAccountStruct.owner, solidAccountStruct.number, heldMarketId, no_deps_constants_1.ZERO_BI);
