...
describe('MagicGLPWrapperTraderV2', () => {
    ...
    before(async () => {
        ...
        wrapper = await (0, abracadabra_1.createMagicGLPWrapperTraderV2)(core);
            ...
            const actions = [
                {
                    ...
                    otherAccountId: 0,
                    data: ethers_1.ethers.utils.defaultAbiCoder.encode(['uint256', 'bytes'], [amountWei, []]),
                },
            ];
    ...
    describe('#exchange', () => {
        ...
        it('should fail if input token is not compatible with GLP', async () => {
            ...
            await (0, assertions_1.expectThrow)(wrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, magicGlp.address, core.tokens.dfsGlp.address, usableUsdcAmount, abiCoder.encode(['uint256'], [no_deps_constants_1.ZERO_BI])), `MagicGLPWrapperTraderV2: Invalid input token <${core.tokens.dfsGlp.address.toLowerCase()}>`);
        });
        it('should fail if output token is incorrect', async () => {
            const dolomiteMarginImpersonator = await (0, utils_1.impersonate)(core.dolomiteMargin.address, true);
            await (0, assertions_1.expectThrow)(wrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, core.tokens.weth.address, core.tokens.usdc.address, amountWei, abiCoder.encode(['uint256'], [otherAmountWei])), `MagicGLPWrapperTraderV2: Invalid output token <${core.tokens.weth.address.toLowerCase()}>`);
        });
        it('should fail if input amount is 0', async () => {
            const dolomiteMarginImpersonator = await (0, utils_1.impersonate)(core.dolomiteMargin.address, true);
            await (0, assertions_1.expectThrow)(wrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, magicGlp.address, core.tokens.usdc.address, no_deps_constants_1.ZERO_BI, abiCoder.encode(['uint256'], [otherAmountWei])), 'MagicGLPWrapperTraderV2: Invalid input amount');
        });
    });
    describe('#MAGIC_GLP', () => {
    ...
    describe('#getExchangeCost', () => {
        ...
        it('should fail if the input token is not in GLP', async () => {
            await (0, assertions_1.expectThrow)(wrapper.getExchangeCost(core.tokens.dfsGlp.address, magicGlp.address, usableUsdcAmount, no_deps_constants_1.BYTES_EMPTY), `MagicGLPWrapperTraderV2: Invalid input token <${core.tokens.dfsGlp.address.toLowerCase()}>`);
        });
        it('should fail if the output token is not dfsGLP', async () => {
            await (0, assertions_1.expectThrow)(wrapper.getExchangeCost(core.tokens.usdc.address, core.tokens.weth.address, usableUsdcAmount, no_deps_constants_1.BYTES_EMPTY), `MagicGLPWrapperTraderV2: Invalid output token <${core.tokens.weth.address.toLowerCase()}>`);
        });
        it('should fail if the input amount is 0', async () => {
            await (0, assertions_1.expectThrow)(wrapper.getExchangeCost(core.tokens.usdc.address, magicGlp.address, no_deps_constants_1.ZERO_BI, no_deps_constants_1.BYTES_EMPTY), 'MagicGLPWrapperTraderV2: Invalid desired input amount');
        });
    });
});
