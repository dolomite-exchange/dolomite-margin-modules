...
describe('MagicGLPUnwrapperTraderV2', () => {
            ...
            const actions = [
                {
                    ...
                    accountId: 0,
                    amount: { sign: false, value: amountWei, ref: src_1.AmountReference.Delta, denomination: src_1.AmountDenomination.Wei },
                    primaryMarketId: marketId,
                    secondaryMarketId: core.marketIds.usdc,
                    otherAddress: unwrapper.address,
                    otherAccountId: 0,
                    data: ethers_1.ethers.utils.defaultAbiCoder.encode(['uint256', 'bytes'], [amountOut, []]),
                },
            ];
    ...
    describe('#exchange', () => {
        ...
        it('should fail if input token is incorrect', async () => {
            ...
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, core.tokens.usdc.address, core.tokens.weth.address, amountWei, no_deps_constants_1.BYTES_EMPTY), `MagicGLPUnwrapperTraderV2: Invalid input token <${core.tokens.weth.address.toLowerCase()}>`);
        });
        it('should fail if output token is incorrect', async () => {
            const dolomiteMarginImpersonator = await (0, utils_1.impersonate)(core.dolomiteMargin.address, true);
            await core.gmxEcosystem.sGlp.connect(core.hhUser1).transfer(unwrapper.address, amountWei);
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, core.tokens.dfsGlp.address, magicGlp.address, amountWei, abiCoder.encode(['uint256'], [otherAmountWei])), `MagicGLPUnwrapperTraderV2: Invalid output token <${core.tokens.dfsGlp.address.toLowerCase()}>`);
        });
        it('should fail if the input amount is 0', async () => {
            const dolomiteMarginImpersonator = await (0, utils_1.impersonate)(core.dolomiteMargin.address, true);
            await core.gmxEcosystem.sGlp.connect(core.hhUser1).transfer(unwrapper.address, amountWei);
            await (0, assertions_1.expectThrow)(unwrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, core.tokens.usdc.address, magicGlp.address, no_deps_constants_1.ZERO_BI, abiCoder.encode(['uint256'], [otherAmountWei])), 'MagicGLPUnwrapperTraderV2: Invalid input amount');
        });
    });
    describe('#MAGIC_GLP', () => {
        it('should work', async () => {
            (0, chai_1.expect)(await unwrapper.MAGIC_GLP()).to.eq(magicGlp.address);
        });
    });
    ...
    describe('#getExchangeCost', () => {
        ...
        it('should fail if the input token is not dsfGLP', async () => {
            await (0, assertions_1.expectThrow)(unwrapper.getExchangeCost(core.tokens.weth.address, core.tokens.usdc.address, amountWei, no_deps_constants_1.BYTES_EMPTY), `MagicGLPUnwrapperTraderV2: Invalid input token <${core.tokens.weth.address.toLowerCase()}>`);
        });
        it('should fail if the output token is not USDC', async () => {
            await (0, assertions_1.expectThrow)(unwrapper.getExchangeCost(magicGlp.address, core.tokens.dfsGlp.address, amountWei, no_deps_constants_1.BYTES_EMPTY), `MagicGLPUnwrapperTraderV2: Invalid output token <${core.tokens.dfsGlp.address.toLowerCase()}>`);
        });
        it('should fail if the input token is not 0', async () => {
            await (0, assertions_1.expectThrow)(unwrapper.getExchangeCost(magicGlp.address, core.tokens.usdc.address, no_deps_constants_1.ZERO_BI, no_deps_constants_1.BYTES_EMPTY), 'MagicGLPUnwrapperTraderV2: Invalid desired input amount');
        });
    });
});
