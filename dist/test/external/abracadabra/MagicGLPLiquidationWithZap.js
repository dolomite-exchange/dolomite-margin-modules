...
describe('MagicGLPLiquidationWithZap', () => {
    ...
    describe('Perform liquidation with full integration', () => {
        it('should work when liquid account is borrowing the output token (USDC)', async () => {
            ...
            const zapOutputs = await zap.getSwapExactTokensForTokensParams(magicGlpApiToken, (0, liquidation_utils_1.toZapBigNumber)(heldUpdatedWithReward), core.apiTokens.usdc, (0, liquidation_utils_1.toZapBigNumber)(usdcDebtAmount), core.hhUser5.address);
        ...
        it('should work when liquid account is borrowing a different output token (WETH)', async () => {
            ...
            const wethLiquidatorBalanceBefore = await core.tokens.weth.connect(core.hhUser1)
                .balanceOf(core.liquidatorProxyV4.address);
            const zapOutputs = await zap.getSwapExactTokensForTokensParams(magicGlpApiToken, (0, liquidation_utils_1.toZapBigNumber)(heldUpdatedWithReward), core.apiTokens.weth, (0, liquidation_utils_1.toZapBigNumber)(wethDebtAmount), core.hhUser5.address);
            const isSuccessful = await (0, liquidation_utils_1.checkForParaswapSuccess)((0, liquidation_utils_1.liquidateV4WithZap)(core, solidAccountStruct, liquidAccountStruct, zapOutputs, no_deps_constants_1.NO_EXPIRY));
            if (!isSuccessful) {
                return;
            }
            await (0, assertions_1.expectProtocolBalance)(core, solidAccountStruct.owner, solidAccountStruct.number, heldMarketId, no_deps_constants_1.ZERO_BI);
    ...
    describe('Perform expiration with full integration', () => {
        ...
        it('should work when expired account is borrowing a different output token (WETH)', async () => {
            ...
            const wethLiquidatorBalanceBefore = await core.tokens.weth.connect(core.hhUser1)
                .balanceOf(core.liquidatorProxyV4.address);
            const zapOutputs = await zap.getSwapExactTokensForTokensParams(magicGlpApiToken, (0, liquidation_utils_1.toZapBigNumber)(heldUpdatedWithReward), core.apiTokens.weth, (0, liquidation_utils_1.toZapBigNumber)(wethDebtAmount), core.hhUser5.address);
            const isSuccessful = (0, liquidation_utils_1.checkForParaswapSuccess)((0, liquidation_utils_1.liquidateV4WithZap)(core, solidAccountStruct, liquidAccountStruct, zapOutputs, expiry));
            if (!isSuccessful) {
                return;
            }
            await (0, assertions_1.expectProtocolBalance)(core, solidAccountStruct.owner, solidAccountStruct.number, heldMarketId, no_deps_constants_1.ZERO_BI);
