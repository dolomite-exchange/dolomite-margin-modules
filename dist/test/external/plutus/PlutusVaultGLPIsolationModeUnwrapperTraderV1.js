...
describe('PlutusVaultGLPIsolationModeUnwrapperTraderV1', () => {
    ...
    describe('#getExchangeCost', () => {
        ...
        it('should fail if the input token is not dsfGLP', async () => {
            await (0, assertions_1.expectThrow)(unwrapper.getExchangeCost(core.tokens.weth.address, core.tokens.usdc.address, amountWei, no_deps_constants_1.BYTES_EMPTY), `PlutusVaultGLPUnwrapperV1: Invalid input token <${core.tokens.weth.address.toLowerCase()}>`);
        });
        it('should fail if the output token is not USDC', async () => {
            await (0, assertions_1.expectThrow)(unwrapper.getExchangeCost(factory.address, core.tokens.weth.address, amountWei, no_deps_constants_1.BYTES_EMPTY), `PlutusVaultGLPUnwrapperV1: Invalid output token <${core.tokens.weth.address.toLowerCase()}>`);
        });
        it('should fail if the desired input amount is eq to 0', async () => {
            await (0, assertions_1.expectThrow)(unwrapper.getExchangeCost(factory.address, core.tokens.usdc.address, no_deps_constants_1.ZERO_BI, no_deps_constants_1.BYTES_EMPTY), 'PlutusVaultGLPUnwrapperV1: Invalid desired input amount');
        });
    });
});
