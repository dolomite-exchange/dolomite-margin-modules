...
describe('PlutusVaultGLPIsolationModeWrapperTraderV1', () => {
    ...
    describe('#exchange', () => {
        ...
        it('should fail if input token is not whitelisted', async () => {
            ...
            await (0, assertions_1.expectThrow)(wrapper.connect(dolomiteMarginImpersonator).exchange(core.hhUser1.address, core.dolomiteMargin.address, factory.address, OTHER_ADDRESS, usableUsdcAmount, abiCoder.encode(['uint256'], [no_deps_constants_1.ZERO_BI])), `PlutusVaultGLPWrapperV1: Invalid input token <${OTHER_ADDRESS.toLowerCase()}>`);
        });
    ...
    describe('#plutusVaultRegistry', () => {
        it('should work', async () => {
            (0, chai_1.expect)(await wrapper.PLUTUS_VAULT_REGISTRY()).to.eq(plutusVaultRegistry.address);
        });
    });
    ...
    describe('#getExchangeCost', () => {
        ...
        it('should fail if the input token is not whitelisted', async () => {
            await (0, assertions_1.expectThrow)(wrapper.getExchangeCost(OTHER_ADDRESS, factory.address, usableUsdcAmount, no_deps_constants_1.BYTES_EMPTY), `PlutusVaultGLPWrapperV1: Invalid input token <${OTHER_ADDRESS.toLowerCase()}>`);
        });
        it('should fail if the output token is not dplvGLP', async () => {
            await (0, assertions_1.expectThrow)(wrapper.getExchangeCost(core.tokens.usdc.address, OTHER_ADDRESS, usableUsdcAmount, no_deps_constants_1.BYTES_EMPTY), `PlutusVaultGLPWrapperV1: Invalid output token <${OTHER_ADDRESS.toLowerCase()}>`);
        });
        it('should fail if the input amount is 0', async () => {
            const dolomiteMarginImpersonator = await (0, utils_1.impersonate)(core.dolomiteMargin.address, true);
            await (0, assertions_1.expectThrow)(wrapper.connect(dolomiteMarginImpersonator).getExchangeCost(core.tokens.usdc.address, factory.address, no_deps_constants_1.ZERO_BI, abiCoder.encode(['uint256'], [no_deps_constants_1.ZERO_BI])), 'PlutusVaultGLPWrapperV1: Invalid desired input amount');
        });
    });
});
