...
describe('PendleYtGLP2024IsolationModeWrapperTraderV2', () => {
  ...
  before(async () => {
    ...
    await core
      .gmxEcosystem!.sGlp.connect(core.hhUser1)
    ...
    await core
      .pendleEcosystem!.ytGlpToken.connect(core.hhUser1)
      .approve(vault.address, amountWei);
    ...
    expect(
      await underlyingToken.connect(core.hhUser1).balanceOf(vault.address)
    ).to.eq(amountWei);
    expect(
      (
  ...
  describe('Call and Exchange for non-liquidation sale', () => {
    it('should work when called with the normal conditions', async () => {
      ...
      const glpAmount = await core
        .gmxEcosystem!.live.glpIsolationModeWrapperTraderV1!.connect(
          core.hhUser5
        )
        .getExchangeCost(
      ...
      await core.tokens.usdc
        .connect(vaultImpersonater)
      ...
      await core.tokens.usdc
        ...
        .transfer(core.dolomiteMargin.address, usableUsdcAmount);
      ...
      await core.dolomiteMargin
        ...
        .operate([defaultAccount], actions);

      const expectedTotalBalance = amountWei.add(approxParams.guessOffchain);
      const underlyingBalanceWei = await core.dolomiteMargin.getAccountWei(
        defaultAccount,
        underlyingMarketId
      );
      ...
      const otherBalanceWei = await core.dolomiteMargin.getAccountWei(
        defaultAccount,
        core.marketIds.usdc
      );
  ...
  describe('#exchange', () => {
    ...
    it('should fail if trade originator is not a vault', async () => {
      const dolomiteMarginImpersonator = await impersonate(
        core.dolomiteMargin.address,
        true
      );
      await expectThrow(
