...
describe('UpgradeableIsolationModeWrapperTrader', () => {
  ...
  let wrapper: TestUpgradeableIsolationModeWrapperTrader;
  ...
  before(async () => {
    ...
    const wrapperImplementation = await createContractWithAbi(
  ...
  describe('#initializer', () => {
    ...
    it('should fail when called again', async () => {
      await expectThrow(
        wrapper.initialize(otherToken.address, factory.address, core.dolomiteMargin.address),
        'Initializable: contract is already initialized',
      );

      await expectThrow(
        wrapper.initializeWrapperTrader(factory.address, core.dolomiteMargin.address),
        'Initializable: contract is already initialized',
      );
    });
  });
  ...
  describe('#exchange', () => {
    ...
    it('should fail if the input token is invalid', async () => {
      const dolomiteMarginImpersonator = await impersonate(core.dolomiteMargin.address, true);
      await expectThrow(
        wrapper.connect(dolomiteMarginImpersonator).exchange(
          vault.address,
        ...
        `IsolationModeWrapperTraderV2: Invalid input token <${core.tokens.wbtc.address.toLowerCase()}>`,
      );
    });

    it('should fail if the input amount is zero', async () => {
      const dolomiteMarginImpersonator = await impersonate(core.dolomiteMargin.address, true);
      await expectThrow(
        wrapper.connect(dolomiteMarginImpersonator).exchange(
          vault.address,
