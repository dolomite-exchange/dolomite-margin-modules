...
abstract contract WrappedTokenUserVaultWrapperTrader is IDolomiteMarginWrapperTrader, OnlyDolomiteMargin {
    ...
    returns (IDolomiteMargin.ActionArgs[] memory) {
        if (DOLOMITE_MARGIN.getMarketTokenAddress(_owedMarket) == address(VAULT_FACTORY)) { /* FOR COVERAGE TESTING */ }
        Require.that(DOLOMITE_MARGIN.getMarketTokenAddress(_owedMarket) == address(VAULT_FACTORY),
            _FILE,
            "Invalid owed market",
            _owedMarket
        );
