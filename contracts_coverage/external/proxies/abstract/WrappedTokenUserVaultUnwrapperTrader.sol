    ...
    OnlyDolomiteMargin
{
    ...
    onlyDolomiteMargin(msg.sender)
    returns (uint256) {
        if (_takerToken == address(VAULT_FACTORY)) { /* FOR COVERAGE TESTING */ }
        Require.that(_takerToken == address(VAULT_FACTORY),
            _FILE,
            "Invalid taker token",
            _takerToken
        );
...

        {
            ...
            if (balance >= _amountTakerToken) { /* FOR COVERAGE TESTING */ }
            Require.that(balance >= _amountTakerToken,
                _FILE,
        ...
        if (outputAmount >= minMakerAmount) { /* FOR COVERAGE TESTING */ }
        Require.that(outputAmount >= minMakerAmount,
            _FILE,
