    ...
    ProxyContractHelpers
{
    ...
    function initialize(
        ...
        if (IWrappedTokenUserVaultFactory(vaultFactory()).getVaultByAccount(_account) == address(this)) { /* FOR COVERAGE TESTING */ }
        Require.that(IWrappedTokenUserVaultFactory(vaultFactory()).getVaultByAccount(_account) == address(this),
            _FILE,
            "Invalid account",
            _account
        );
