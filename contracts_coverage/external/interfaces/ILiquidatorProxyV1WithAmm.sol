...
interface ILiquidatorProxyV1WithAmm {
     ...
     * @param  _owedMarket                      The owed market whose borrowed value will be added to
     *                                          `owedWeiToLiquidate`
     * @param  _heldMarket                      The held market whose collateral will be recovered to take on the debt
     *                                          of `owedMarket`
     * @param  _tokenPath                       The path through which the trade will be routed to recover the
     *                                          collateral
     * @param  _expiry                          The time at which the position expires, if this liquidation is for
     *                                          closing an expired position. Else, 0.
     * @param  _minOwedOutputAmount             The minimum amount that should be outputted by the trade from heldWei to
     *                                          owedWei. Used to prevent sandwiching and mem-pool other attacks. Only
     *                                          used if `revertOnFailToSellCollateral` is set to `false` and the
     ...
     *                                          liquidation cannot repay the owed debt. False to swallow the error and
     *                                          sell whatever is possible. If set to false, the liquidator must have
     *                                          enough assets to be prevent becoming liquidated or under-collateralized.
     */
    function liquidate(
