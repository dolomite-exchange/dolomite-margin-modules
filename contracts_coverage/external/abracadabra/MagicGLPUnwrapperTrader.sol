...
contract MagicGLPUnwrapperTrader is IDolomiteMarginUnwrapperTrader, OnlyDolomiteMargin {
    ...
    returns (uint256) {
        if (_takerToken == address(MAGIC_GLP)) { /* FOR COVERAGE TESTING */ }
        Require.that(_takerToken == address(MAGIC_GLP),
            _FILE,
            "Invalid taker token",
            _takerToken
        );
        if (GMX_REGISTRY.gmxVault().whitelistedTokens(_makerToken)&& DOLOMITE_MARGIN.getMarketIdByTokenAddress(_makerToken) == outputMarketId()) { /* FOR COVERAGE TESTING */ }
        Require.that(GMX_REGISTRY.gmxVault().whitelistedTokens(_makerToken)
    ...
    view
    returns (uint256) {
        if (_makerToken == address(MAGIC_GLP)) { /* FOR COVERAGE TESTING */ }
        Require.that(_makerToken == address(MAGIC_GLP),
            _FILE,
            "Invalid maker token",
            _makerToken
        );
        if (GMX_REGISTRY.gmxVault().whitelistedTokens(_takerToken)&& DOLOMITE_MARGIN.getMarketIdByTokenAddress(_takerToken) == outputMarketId()) { /* FOR COVERAGE TESTING */ }
        Require.that(GMX_REGISTRY.gmxVault().whitelistedTokens(_takerToken)
