...
contract Vester is OnlyDolomiteMargin, ReentrancyGuard, IVester {
    ...
    returns (uint256) {
        ...
        if (_duration >= _MIN_DURATION && _duration <= _MAX_DURATION && _duration % _DURATION_INTERVAL == 0) { /* FOR COVERAGE TESTING */ }
        Require.that(_duration >= _MIN_DURATION && _duration <= _MAX_DURATION && _duration % _DURATION_INTERVAL == 0,
            _FILE,
            "Invalid duration"
        );
    ...
    payable {
        ...
        if (block.timestamp <= _position.startTime + _position.duration + _DURATION_INTERVAL) { /* FOR COVERAGE TESTING */ }
        Require.that(block.timestamp <= _position.startTime + _position.duration + _DURATION_INTERVAL,
            _FILE,
            "Position expired"
        );
    ...
    onlyDolomiteMarginGlobalOperator(msg.sender) {
        ...
        if (block.timestamp > _position.startTime + _position.duration + 1 weeks) { /* FOR COVERAGE TESTING */ }
        Require.that(block.timestamp > _position.startTime + _position.duration + 1 weeks,
            _FILE,
