    ...
    IsolationModeTokenVaultV1WithFreezableAndPausable
{
    ...
    onlyLiquidator(msg.sender) {
        ...
        if (_inputAmount == DOLOMITE_MARGIN().getAccountWei(account, marketId()).value) { /* FOR COVERAGE TESTING */ }
        Require.that(_inputAmount == DOLOMITE_MARGIN().getAccountWei(account, marketId()).value,
            _FILE,
            "Invalid inputAmount"
        );
        _initiateUnwrapping(
    ...
    function _initiateUnwrapping(
        ...
        if (registry().umamiUnwrapperTrader().isValidOutputToken(_outputToken)) { /* FOR COVERAGE TESTING */ }
        Require.that(registry().umamiUnwrapperTrader().isValidOutputToken(_outputToken),
            _FILE,
