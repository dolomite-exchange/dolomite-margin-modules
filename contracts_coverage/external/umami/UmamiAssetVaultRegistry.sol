...
contract UmamiAssetVaultRegistry is IUmamiAssetVaultRegistry, BaseRegistry {
    ...
    external {
        if (msg.sender == address(umamiUnwrapperTrader())) { /* FOR COVERAGE TESTING */ }
        Require.that(msg.sender == address(umamiUnwrapperTrader()),
            _FILE,
    ...
    function _ownerSetStorageViewer(address _storageViewer) internal {
        ...
        Require.that(_storageViewer != address(0),
            _FILE,
