    ...
    UpgradeableIsolationModeUnwrapperTrader
{
    ...
    function vaultSetWithdrawalInfo(
        ...
        if (!UMAMI_REGISTRY().isAccountWaitingForCallback(msg.sender, _accountNumber)) { /* FOR COVERAGE TESTING */ }
        Require.that(!UMAMI_REGISTRY().isAccountWaitingForCallback(msg.sender, _accountNumber),
            _FILE,
