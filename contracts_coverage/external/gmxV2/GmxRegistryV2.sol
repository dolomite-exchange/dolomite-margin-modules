...
contract GmxRegistryV2 is IGmxRegistryV2, BaseRegistry {
    ...
    function initializeTraders(
        ...
        Require.that(address(gmxV2UnwrapperTrader()) == address(0) && address(gmxV2WrapperTrader()) == address(0),
            _FILE,
            "Already initialized"
        );
    ...
    external {
        if (msg.sender == address(gmxV2UnwrapperTrader()) || msg.sender == address(gmxV2WrapperTrader())) { /* FOR COVERAGE TESTING */ }
        Require.that(msg.sender == address(gmxV2UnwrapperTrader()) || msg.sender == address(gmxV2WrapperTrader()),
            _FILE,
