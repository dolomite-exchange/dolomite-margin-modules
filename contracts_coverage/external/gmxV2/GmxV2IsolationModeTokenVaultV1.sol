...
//    }
    ...
    override {
        ...
        if (_tradersPath[len - 1].traderType == IGenericTraderBase.TraderType.IsolationModeWrapper) {
            ...
            if (_tradersPath[len - 1].trader == address(registry().gmxV2WrapperTrader())) { /* FOR COVERAGE TESTING */ }
            Require.that(_tradersPath[len - 1].trader == address(registry().gmxV2WrapperTrader()),
                _FILE,
                "Invalid trader"
            );
            _tradersPath[len - 1].tradeData = abi.encode(_tradeAccountNumber, msg.value);
