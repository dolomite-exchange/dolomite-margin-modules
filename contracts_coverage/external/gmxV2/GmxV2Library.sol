...
library GmxV2Library {
    ...
    function validateExecutionFee(IGmxV2IsolationModeTokenVaultV1 _vault, uint256 _toAccountNumber) public view {
        ...
        Require.that(msg.value == IGmxV2IsolationModeVaultFactory(factory).executionFee(),
            _FILE,
            "Invalid execution fee"
        );
        if (_vault.getExecutionFeeForAccountNumber(_toAccountNumber) == 0) { /* FOR COVERAGE TESTING */ }
        Require.that(_vault.getExecutionFeeForAccountNumber(_toAccountNumber) == 0,
            _FILE,
    ...
    function checkVaultAccountIsNotFrozen(
        ...
        if (!_factory.isVaultAccountFrozen(_vault, _accountNumber)) { /* FOR COVERAGE TESTING */ }
        Require.that(!_factory.isVaultAccountFrozen(_vault, _accountNumber),
            _FILE,
            "Account is frozen",
            _vault,
            _accountNumber
