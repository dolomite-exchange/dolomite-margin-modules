...
contract GmxV2LiquidatorProxy is BaseLiquidatorProxy {
    ...
    function prepareForLiquidation(
        ...
        if (IGmxV2IsolationModeVaultFactory(gmToken).getAccountByVault(_liquidAccount.owner) != address(0)) { /* FOR COVERAGE TESTING */ }
        Require.that(IGmxV2IsolationModeVaultFactory(gmToken).getAccountByVault(_liquidAccount.owner) != address(0),
            _FILE,
    ...
    function _checkIsLiquidatable(
        ...
        if (_expirationTimestamp != 0) {
            ...
            if (DOLOMITE_MARGIN.getAccountStatus(_liquidAccount) == IDolomiteStructs.AccountStatus.Liquid ||!_isCollateralized(liquidSupplyValue.value, liquidBorrowValue.value, DOLOMITE_MARGIN.getMarginRatio())) { /* FOR COVERAGE TESTING */ }
