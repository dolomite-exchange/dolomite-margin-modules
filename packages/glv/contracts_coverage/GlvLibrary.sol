...
library GlvLibrary {
    ...
    function initiateCancelDeposit(IGlvIsolationModeWrapperTraderV2 _wrapper, bytes32 _key) public {
        ...
        if (msg.sender == depositInfo.vault || IAsyncIsolationModeTraderBase(address(_wrapper)).isHandler(msg.sender)) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function unwrapperExecuteInitiateUnwrapping(
        ...
        if (_minOutputAmount > 0 && minOtherTokenAmount > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function validateEventDataForWithdrawal(
        ...
        if (_marketTokenAmount >= _withdrawalInfo.inputAmount) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function _validateEventData(string memory _foundKey, string memory _expectedKey) private pure {
        if (keccak256(abi.encodePacked(_foundKey)) == keccak256(abi.encodePacked(_expectedKey))) { /* FOR COVERAGE TESTING */ }
        Require.that(
