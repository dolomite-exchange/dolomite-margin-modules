...
contract FinalSettlementViaInternalSwapProxy is OnlyDolomiteMargin {
    ...
    function ownerForceWithdraw(
        ...
        if (_accounts.length > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(
            _accounts.length > 0,
            _FILE,
            "Invalid accounts"
    ...
    function ownerSettle(
        ...
        if (_borrowAccounts.length == _supplyAccounts.length && _borrowAccounts.length > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(
            _borrowAccounts.length == _supplyAccounts.length && _borrowAccounts.length > 0,
            _FILE,
            "Invalid accounts"
        );
        if (_owedMarket != _rewardMarket) { /* FOR COVERAGE TESTING */ }
        Require.that(
            _owedMarket != _rewardMarket,
            _FILE,
            "Invalid markets"
        );
        if (_reward.value > 0 && _reward.value <= DOLOMITE_MARGIN().getLiquidationSpread().value) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function _settleAccount(
        ...
        if (!owedBalance.sign && owedBalance.value != 0) { /* FOR COVERAGE TESTING */ }
        Require.that(
        ...
        if (heldBalance.sign && heldBalance.value != 0) { /* FOR COVERAGE TESTING */ }
        Require.that(
