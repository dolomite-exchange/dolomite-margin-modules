...
contract DolomiteOwner is IDolomiteOwner, AccessControl {
    ...
    modifier notNull(address _address) {
        if (_address != ADDRESS_ZERO) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    modifier transactionExists(uint256 transactionId) {
        if (transactions[transactionId].destination != ADDRESS_ZERO) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function ownerAddRoleAddresses(
        ...
        if (_roles.contains(_role)) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function executeTransaction(
        ...
        if (hasRole(EXECUTOR_ROLE, msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) { /* FOR COVERAGE TESTING */ }
        Require.that(
