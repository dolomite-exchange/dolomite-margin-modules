    ...
    Initializable
{
    ...
    function _ownerSetAdminRegistry(
        ...
        if (_adminRegistry != address(0)) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    function _ownerSetBlacklistedAddresses(
        ...
        if (_blacklistedAddresses.length == _isBlacklisted.length) { /* FOR COVERAGE TESTING */ }
        Require.that(
        ...
        for (uint256 i; i < _blacklistedAddresses.length; ++i) {
            if (_blacklistedAddresses[i] != address(0)) { /* FOR COVERAGE TESTING */ }
            Require.that(
    ...
    function _ownerSetDao(
        ...
        if (_dao != address(0)) { /* FOR COVERAGE TESTING */ }
        Require.that(
            _dao != address(0),
            _FILE,
            "Invalid dao"
