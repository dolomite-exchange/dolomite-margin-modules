 ...
 */
contract DepositWithdrawalRouter is OnlyDolomiteMargin, ReentrancyGuard, IDepositWithdrawalRouter {
    ...
    nonReentrant {
        ...
        if (marketInfo.isIsolationModeAsset) {
            _depositIntoIsolationModeMarket(
                marketInfo.factory,
                marketInfo.token,
                _marketId,
                amount
            ...
            _depositWeiIntoDolomiteMargin(marketInfo.token, amount, _marketId, _toAccountNumber);
        }
    }
    ...
    nonReentrant {
        ...
        if (marketInfo.isIsolationModeAsset) {
            ...
            uint256 amountWei = _convertParToWei(_toAccountNumber, _marketId, _amountPar);
            marketInfo.token.safeTransferFrom(msg.sender, address(this), amountWei);
            // @follow-up Should deposit as wei or par? Wei seems safer so we use full amountWei no matter what
            _depositWeiIntoDolomiteMargin(marketInfo.token, amountWei, _marketId, _toAccountNumber);
        }
    }
    ...
    function _depositIntoIsolationModeMarket(
        IIsolationModeVaultFactory _factory,
        IERC20 _underlyingToken,
        uint256 _marketId,
        uint256 _amount
    ) internal {
        address vault = _factory.getVaultByAccount(msg.sender);
        ...
        _factory.enqueueTransferIntoDolomiteMargin(vault, _amount);
        IERC20(address(_factory)).safeApprove(address(DOLOMITE_MARGIN()), _amount);
        _underlyingToken.safeApprove(vault, _amount);
    ...
    function _depositWeiIntoDolomiteMargin(
        IERC20 _token,
        uint256 _amount,
        uint256 _marketId,
        uint256 _toAccountNumber
    ) internal {
        _token.safeApprove(address(DOLOMITE_MARGIN()), _amount);
        AccountActionLib.deposit(
            DOLOMITE_MARGIN(),
            /* _accountOwner = */ msg.sender, // solium-disable-line indentation
            /* _fromAccount = */ address(this), // solium-disable-line indentation
    ...
    function _getMarketInfo(
        ...
        if (!isSuccess || !abi.decode(returnData, (bool))) {
            return MarketInfo({
