...
contract OkxAggregatorTrader is AggregatorTraderBase {
    ...
    returns (uint256) {
        ERC20Lib.resetAllowanceIfNeededAndApprove(IERC20(_inputToken), address(OKX_AGGREGATOR), _inputAmount);
    ...
    function _overwriteInputAmountAndCall(
        ...
        if (_okxFunctionSelector == PMMV2_SWAP_SELECTOR) {
            ...
            baseRequest.minReturnAmount = _getScaledExpectedOutputAmount(baseRequest.fromTokenAmount, _inputAmount, baseRequest.minReturnAmount);
            ...
            (
                ...
                _okxCallData,
                (IOkxAggregator.DexBaseRequest, uint256[], IOkxAggregator.DexRouterPath[][], IOkxAggregator.PMMSwapRequest[], address)
            );
            baseRequest.minReturnAmount = _getScaledExpectedOutputAmount(baseRequest.fromTokenAmount, _inputAmount, baseRequest.minReturnAmount);
            baseRequest.fromTokenAmount = _inputAmount;
            ...
            (
                ...
                _okxCallData,
                (uint256, IOkxAggregator.DexBaseRequest, uint256[], IOkxAggregator.DexRouterPath[][], IOkxAggregator.PMMSwapRequest[])
            );
            baseRequest.minReturnAmount = _getScaledExpectedOutputAmount(baseRequest.fromTokenAmount, _inputAmount, baseRequest.minReturnAmount);
            baseRequest.fromTokenAmount = _inputAmount;
