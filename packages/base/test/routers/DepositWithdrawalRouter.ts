...
describe('DepositWithdrawalRouter', () => {
  ...
  before(async () => {
    ...
    router = await createTestDepositWithdrawalRouter(core, core.tokens.payableToken);
  ...
  describe('#constructor', () => {
    it('should work normally', async () => {
      expect(await router.isInitialized()).to.be.true;
      expect(await router.wrappedPayableToken()).to.equal(core.tokens.weth.address);
      expect(await router.payableMarketId()).to.equal(core.marketIds.weth);
  ...
  describe('#ownerLazyInitialize', () => {
    it('should fail if not owner', async () => {
      await expectThrow(
        router.connect(core.hhUser1).ownerLazyInitialize(core.tokens.weth.address),
  ...
  describe('#depositWei', () => {
    ...
    it('should allow paying back debt when borrowed asset has supply cap', async () => {
      ...
      await expectThrow(
        router.depositWei(isolationModeMarketId, BigNumber.from('124'), core.marketIds.dai, amountWei, EventFlag.None),
