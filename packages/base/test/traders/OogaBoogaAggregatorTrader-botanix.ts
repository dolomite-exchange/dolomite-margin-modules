...
const amountIn = parseEther('.001');

describe('OogaBoogaAggregatorTrader-botanix', () => {
  ...
  before(async () => {
    core = await setupCoreProtocol({
      blockNumber: await getRealLatestBlockNumber(true, Network.Botanix),
    ...
    await setupPBTCBalance(core, core.hhUser1, amountIn, { address: core.dolomiteMargin.address });
    await depositIntoDolomiteMargin(core, core.hhUser1, defaultAccountNumber, core.marketIds.pbtc, amountIn);
  ...
  describe('#exchange', () => {
    ...
    it('should succeed for normal swap when inputAmount is lower but output is still sufficient', async () => {
      ...
      expect(await core.tokens.pbtc.balanceOf(trader.address)).to.eq(ZERO_BI);
      expect(await core.tokens.usdc.balanceOf(trader.address)).to.eq(ZERO_BI);
      await expectProtocolBalanceDustyOrZero(core, core.hhUser1, defaultAccountNumber, core.marketIds.pbtc);
      await expectProtocolBalanceIsGreaterThan(core, defaultAccount, core.marketIds.usdc, outputAmount, 0);
    });
  ...
  describe('#getExchangeCost', () => {
    it('should always fail', async () => {
      await expectThrow(
        trader.getExchangeCost(core.tokens.pbtc.address, core.tokens.usdc.address, ZERO_BI, BYTES_EMPTY),
