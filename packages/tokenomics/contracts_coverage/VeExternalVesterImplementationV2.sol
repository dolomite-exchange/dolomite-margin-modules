    ...
    IVeExternalVesterV2
{
        ...
        returns (uint256)
    {
        ...
        if (_oTokenAmount <= _maxPairAmount) { /* FOR COVERAGE TESTING */ }
        Require.that(
    ...
    returns (uint256) {
        ...
        if (_veTokenId == type(uint256).max || VE_TOKEN.ownerOf(_veTokenId) == positionOwner) { /* FOR COVERAGE TESTING */ }
        Require.that(
        ...
        if (_veTokenId != type(uint256).max) {
            if (_veLockEndTime == 0) { /* FOR COVERAGE TESTING */ }
            Require.that(
                _veLockEndTime == 0,
                _FILE,
            ...
            if (_veLockEndTime > block.timestamp && _veLockEndTime % 1 weeks == 0) { /* FOR COVERAGE TESTING */ }
            Require.that(
