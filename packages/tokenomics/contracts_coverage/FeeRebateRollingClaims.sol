...
contract FeeRebateRollingClaims is BaseClaim, IFeeRebateRollingClaims {
    ...
    function handlerSetMerkleRoots(
        ...
        if (_marketIds.length == _merkleRoots.length && _merkleRoots.length == _totalAmounts.length) { /* FOR COVERAGE TESTING */ }
        Require.that(
        ...
        if (_getFeeRebateRollingClaimsStorage().currentEpoch + 1 == _expectedEpoch) { /* FOR COVERAGE TESTING */ }
        Require.that(
