    ...
    IExternalVesterV1
{
        ...
        initializer
    {
        ...
        _ownerSetDiscountCalculator(_oToken);
        _ownerSetOToken(_oToken);
        _ownerSetOwner(_owner);
        _ownerSetClosePositionWindow(4 weeks);
    ...
    {
        Require.that(
            pushedTokens() >= _amount + promisedTokens(),
            _FILE,
            "Not enough rewards available"
        );
        ...
        emit VestingStarted(msg.sender, 0, _amount, 0, nftId);
    ...
    function ownerAccrueRewardTokenInterest() external onlyOwner(msg.sender) {
        ...
        AccountActionLib.transfer(
            ...
            type(uint256).max,
            IDolomiteStructs.BalanceCheckFlag.Both
        );
    }
    ...
    function _getRewardPriceAdj(uint256 _duration) internal pure returns (uint256) {
