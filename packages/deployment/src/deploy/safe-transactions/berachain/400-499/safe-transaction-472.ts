...
async function main(): Promise<DryRunOutput<Network.Berachain>> {
  ...
  const registryImplementationAddress = await deployContractAndSave(
    ...
    [],
    'EventEmitterRegistryImplementationV7',
  );
  ...
  const veExternalVesterImplementationAddress = await deployContractAndSave(
    ...
    'VeExternalVesterImplementationV4',
  );

  const transactions = [
    await prettyPrintEncodedDataWithTypeSafety(core, core, 'eventEmitterRegistryProxy', 'upgradeTo', [
  ...
  return {
    ...
    invariants: async () => {
      expect(await core.eventEmitterRegistryProxy.implementation()).to.eq(registryImplementationAddress);
