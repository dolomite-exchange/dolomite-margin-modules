...
async function main(): Promise<DryRunOutput<Network.Berachain>> {
  ...
  const implementationAddress = await deployContractAndSave(
    ...
    'VeExternalVesterImplementationV3',
  );
  ...
  return {
    ...
    invariants: async () => {
      ...
      expect(await core.tokenomics.veExternalVester.availableTokens()).to.eq('3277977867219692970654909');
      expect(await core.tokenomics.veDolo.team()).to.eq(core.tokenomics.daoAddress);
    },
  };
}
