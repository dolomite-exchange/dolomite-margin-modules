...
import { MAX_UINT_256_BI, Network, ZERO_BI } from '@dolomite-exchange/modules-base/src/utils/no-deps-constants';
...
async function main(): Promise<DryRunOutput<Network.Berachain>> {
  ...
  const vesterV2Address = await deployContractAndSave(
    ...
    'VeExternalVesterImplementationV8',
  );
  ...
  return {
    ...
    invariants: async () => {
      ...
      assertHardhatInvariant(
        (await core.tokenomics.veExternalVester.promisedTokens())
          .eq(await core.tokens.dolo.balanceOf(core.tokenomics.veExternalVester.address)),
        'Promised tokens should be equal to dolo.balanceOf(vester)',
      );

      const dao = await impersonate(core.daoAddress!, true);
      await core.tokens.dolo.connect(dao).approve(core.tokenomics.veExternalVester.address, MAX_UINT_256_BI);
      assertHardhatInvariant(
        (await core.tokens.dolo.balanceOf(core.daoAddress!)).eq(await core.tokenomics.veExternalVester.pushedTokens()),
