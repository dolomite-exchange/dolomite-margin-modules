...
async function main(): Promise<DryRunOutput<Network.BerachainCartio>> {
  ...
  transactions.push(
    ...(await encodeListing(core, core.tokens.rsEth, '5000000000000000000000')),
  );
  return {
    core,
    upload: {
    ...
    invariants: async () => {
      assertHardhatInvariant(
        (await core.dolomiteMargin.getMarketTokenAddress(core.marketIds.rsEth)) === core.tokens.rsEth.address,
        'Invalid rsEth token',
      );
    },
  };
