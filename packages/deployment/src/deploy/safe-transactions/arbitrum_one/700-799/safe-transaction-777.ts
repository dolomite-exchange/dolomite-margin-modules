 ...
 * - Adds new GLV/ETH GM markets index tokens to the oracle aggregator
 * - Adds new GLV/BTC GM markets index tokens to the oracle aggregator
 *
 * - GLV/ETH -> APT, CRV, CVX, MNT, POL (address changed), SUI, WLFI
 * - GLV/BTC -> BNB, CRO, PUMP
 */
async function main(): Promise<DryRunOutput<Network.ArbitrumOne>> {
  ...
  for (const tokenSymbol of tokenSymbols) {
    ...
    try {
      await core.chainlinkPriceOracleV3.getPrice(tokenAddress);
    } catch (error) {
      console.log(`Pushing transaction for ${tokenSymbol}`);
