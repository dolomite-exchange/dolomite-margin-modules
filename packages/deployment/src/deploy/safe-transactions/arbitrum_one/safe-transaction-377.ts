...
async function main(): Promise<DryRunOutput<Network.ArbitrumOne>> {
  ...
  const weEth = await core.liquidatorAssetRegistry.getLiquidatorsForAsset(core.marketIds.dPtWeEthSep2024);
  const ezEth = await core.liquidatorAssetRegistry.getLiquidatorsForAsset(core.marketIds.dPtEzEthSep2024);
  const rsEth = await core.liquidatorAssetRegistry.getLiquidatorsForAsset(core.marketIds.dPtRsEthSep2024);
  console.log('data', weEth, ezEth, rsEth);
