...
/**
 * @title   BexIsolationModeUnwrapperTraderV2
 * @author  Dolomite
 *
 * @notice  Used for unwrapping from a BEX LP Pair. Upon settlement, the tokens are
 *          sent from the user's vault and the factory token is burned from `DolomiteMargin`.
 */
contract BexIsolationModeUnwrapperTraderV2 is IsolationModeUnwrapperTraderV2 {
    ...
    function isValidOutputToken(address _outputToken) public override view returns (bool) {
        return _getStorage().poolTokens.contains(_outputToken);
    }
    ...
    returns (uint256) {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            if (address(assets[i]) == _outputToken) {
                minAmountsOut[i] = _minOutputAmount;
                exitTokenIndex = i;
            } else {
                minAmountsOut[i] = 0;
            }
        ...
        BEX_VAULT.exitPool(
            ...
            IBexVault.ExitPoolRequest({
                assets: assets,
                minAmountsOut: minAmountsOut,
                userData: abi.encode(
                    IBexVault.StablePoolExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT,
                    _inputAmount,
                    exitTokenIndex - 1
                ),
