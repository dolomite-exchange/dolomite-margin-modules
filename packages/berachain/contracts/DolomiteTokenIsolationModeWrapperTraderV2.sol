...
contract DolomiteTokenIsolationModeWrapperTraderV2 is ProxyContractHelpers, IsolationModeWrapperTraderV2, IDolomiteMarginInternalTrader {
    ...
    // @todo Adjust so we only have to pass in the berachain rewards registry
    constructor(
    ...
    returns (uint256) {
        // @todo require that output token is a market and a dPOL token
        // @todo require that input token is the underlying dToken's asset
        // @todo then check accounts and vaults
        Require.that(
